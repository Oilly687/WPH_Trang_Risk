			...
			<div class="navbar-content scroll-div " >
				...
				<ul class="nav pcoded-inner-navbar ">
					...
					<li class="nav-item">
						...
						<a href="?actionmenu=dm" class="nav-link "><span class="pcoded-micon"><i class="fa-solid fa-fire-flame-simple"></i></span><span class="pcoded-mtext">โรคเบาหวาน</span></a>
						<a href="?actionmenu=psh2" class="nav-link "><span class="pcoded-micon"><i class="feather icon-activity"></i></span><span class="pcoded-mtext">โรคความดันโลหิตสูง</span></a>
				...
				<div class="row align-items-center">
					<div class="col-md-6">
						...
						<ul class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="#!">โรคเบาหวาน</a></li>
...
<div class="col-md-6">
		...
		<div class="row">
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">ข้อมูลทั้งหมดของการประเมินความเสี่ยงโรคเบาหวาาน</div>
					<div class="card-body">
                ...
                <thead>
                  <tr>
                     ...
                     <th>ประวัติความดัน</th>
                     <th>ประวัติเบาหวาน</th>
                     <th>เบอร์โทรศัพท์</th>
                     <th>อีเมล</th>
                     <th>วันที่</th>
                     <th>เวลา</th>
                ...
                <?php
                $emp_query = "SELECT name, gender, dob, weight, height, waisline, risk, pressure, dm, phone, email, time, ti, (YEAR(NOW()) - YEAR(`dob`)) as old  FROM `member` WHERE active = '1'";
                ...
                if(mysqli_num_rows($employeesRecords) > 0){
                    while($empRecord = mysqli_fetch_assoc($employeesRecords)){
                        ...
                        $dob = $empRecord['dob'];
                        $weight = $empRecord['weight'];
                        $height = $empRecord['height'];
                        $waisline = $empRecord['waisline'];
                        $risk = $empRecord['risk'];
                        $pressure = $empRecord['pressure'];
                        $dm = $empRecord['dm'];
...
<?php
$query = "SELECT gender, COUNT(gender) as total FROM member WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
...
?>
	...
	<?php
$query1 = "SELECT risk, COUNT(risk) as total FROM member WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
