				...
				<div class="row align-items-center">
					<div class="col-md-6">
						...
						<ul class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="#!">โรคหลอดเลือดหัวใจ</a></li>
...
<div class="col-md-6">
		...
		<div class="row">
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">ข้อมูลทั้งหมดของการประเมินความเสี่ยงโรคหลอดเลือดหัวใจ</div>
					<div class="card-body">
                ...
                <?php
                $emp_query = "SELECT name, sex, dob, sbp, cholesterol,  risk, dm, smk, phone, email, time, ti, (YEAR(NOW()) - YEAR(`dob`)) as old  FROM `cad` WHERE active = '1'";
                ...
                if(mysqli_num_rows($employeesRecords) > 0){
                    while($empRecord = mysqli_fetch_assoc($employeesRecords)){
                        $name = $empRecord['name'];
                        $gender = $empRecord['sex'];
                        $dob = $empRecord['dob'];
                        $sbp = $empRecord['sbp'];
                        $cholesterol = $empRecord['cholesterol'];
                        $risk = $empRecord['risk'];
                        $dm = $empRecord['dm'];
                        $smk = $empRecord['smk'];
...
<?php
$query = "SELECT sex, COUNT(sex) as total FROM cad WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
...
?>
	...
	<?php
$query1 = "SELECT risk, COUNT(risk) as total FROM cad WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
