				...
				<div class="row align-items-center">
					<div class="col-md-6">
						...
						<ul class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="#!">โรคสมองเสื่อม</a></li>
...
<div class="col-md-6">
		...
		<div class="row">
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">ข้อมูลทั้งหมดของการประเมินความเสี่ยงโรคสมองเสื่อม</div>
					<div class="card-body">
                ...
                <thead>
                  <tr>
                     ...
                     <th>ชื่อ-สกุล ญาติ</th>
                     <th>เพศ</th>
                     <th>DOB</th>
                     <th>ระดับความเสี่ยง</th>
                     <th>จำคนในครอบครัว</th>
                     <th>จำพักได้</th>
                     <th>จำเหตุการณ์ที่สำคัญในอดีต</th>
                     <th>ปรับตัวเข้ากับการเปลี่ยนแปลง</th>
                     <th>เรียนรู้สิ่งใหม่ๆ</th>
                     <th>เข้าใจในสิ่งที่เกิดขึ้น</th>
                     <th>เดินทางโดยลำพัง</th>
                     <th>ทำงานที่เคยทำ</th>
                     <th>มือถือ</th>
                     <th>อีเมล</th>
                     <th>วันที่</th>
                ...
                <?php
                $emp_query = "SELECT nameP, nameC, phone, email, dob, risk, gender, rememF, rememWF, 
                rememH, canRu, canln, canlb, canTra, canW, time, ti, (YEAR(NOW()) - YEAR(`dob`)) as old  FROM `dt` WHERE active = '1'";
                ...
                if(mysqli_num_rows($employeesRecords) > 0){
                    while($empRecord = mysqli_fetch_assoc($employeesRecords)){
                        $nameP = $empRecord['nameP'];
                        $nameC = $empRecord['nameC'];
                        $gender = $empRecord['gender'];
                        $dob = $empRecord['dob'];
                        $risk = $empRecord['risk'];
                        $rememF = $empRecord['rememF'];
                        $rememWF = $empRecord['rememWF'];
                        $rememH = $empRecord['rememH'];
                        $canRu = $empRecord['canRu'];
                        $canln = $empRecord['canln'];
                        $canlb = $empRecord['canlb'];
                        $canTra = $empRecord['canTra'];
                        $canW = $empRecord['canW'];
...
<?php
$query = "SELECT gender, COUNT(gender) as total FROM dt WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
...
?>
	...
	<?php
$query1 = "SELECT risk, COUNT(risk) as total FROM dt WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
