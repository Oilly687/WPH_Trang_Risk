...
//var_dump($_SESSION);
  ...
  $level = $_SESSION['level'];
 	if($level!='member'){
    include $_SERVER['DOCUMENT_ROOT'].'/assessment/AdminSystem_Trang/logout.php';exit;
...
<html lang="en">
<head>
	<title>โรคความดันโลหิตสูง</title>
				...
				<div class="row align-items-center">
					<div class="col-md-6">
						...
						<ul class="breadcrumb">
							...
							<li class="breadcrumb-item"><a href="#!">โรคความดันโลหิตสูง</a></li>
...
<div class="col-md-6">
		...
		<div class="row">
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header">ข้อมูลทั้งหมดของการประเมินความเสี่ยงโรคความดันโลหิตสูง</div>
					<div class="card-body">
              ...
              <thead>
                <tr>
                   ...
                   <th>ดื่มแอลกอฮอล์</th>
                   <th>กินเค็ม</th>
                   <th>ออกกำลังกายน้อย</th>
                   <th>เบาหวาน</th>
                   <th>มีอาการ</th>
                   <th>ความดัน</th>
                   <th>พ่อแม่โรคความดัน</th>
                   <th>มือถือ</th>
                   <th>อีเมล</th>
                   <th>วันที่</th>
                   <th>เวลา</th>
                ...
                <?php
                $emp_query = "SELECT name_ps, gender_ps, dob, risk_ps, age_ps, overS, lgg_ps, kam_ps, exr_ps, dm_ps, rk_ps, psh_ps, spsh_ps, phone_ps, email_ps, time, ti, (YEAR(NOW()) - YEAR(`dob`)) as old FROM `ps` WHERE active = '1'";
                ...
                if(mysqli_num_rows($employeesRecords) > 0){
                    while($empRecord = mysqli_fetch_assoc($employeesRecords)){
                        $name_ps = $empRecord['name_ps'];
                        $gender_ps = $empRecord['gender_ps'];
                        $dob = $empRecord['dob'];
                        $risk_ps = $empRecord['risk_ps'];
                        $age_ps = $empRecord['age_ps'];
                        $overS = $empRecord['overS'];
                        $lgg_ps = $empRecord['lgg_ps'];
                        $kam_ps = $empRecord['kam_ps'];
                        $exr_ps = $empRecord['exr_ps'];
                        $dm_ps = $empRecord['dm_ps'];
                        $rk_ps = $empRecord['rk_ps'];
                        $psh_ps = $empRecord['psh_ps'];
                        $spsh_ps = $empRecord['spsh_ps'];
                        $phone_ps = $empRecord['phone_ps'];
                        $email_ps = $empRecord['email_ps'];
                        $time = $empRecord['time'];
                        $old = $empRecord['old'];
                        $ti = $empRecord['ti'];
...
<?php
$query = "SELECT gender_ps, COUNT(gender_ps) as total FROM ps WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
  ...
  $query  .=  "GROUP BY gender_ps";
$result = mysqli_query($conn, $query);

    $datax = array();
    foreach ($result as $k) {
      $datax[] = "['".$k['gender_ps']."'".", ".$k['total']."]";
    }
...
?>
	...
	<?php
$query1 = "SELECT risk_ps, COUNT(risk_ps) as total FROM ps WHERE active='1'";
if(isset($_POST['but_search'])){
    $fromDate = $_POST['fromDate'];
    $endDate = $_POST['endDate'];
  ...
  $query1  .=  "GROUP BY risk_ps";
$result1 = mysqli_query($conn, $query1);

    $datax1 = array();
    foreach ($result1 as $S) {
      $datax1[] = "['".$S['risk_ps']."'".", ".$S['total']."]";
    }
