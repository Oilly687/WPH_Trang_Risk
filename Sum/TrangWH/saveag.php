...
<?php
	...
	$q_id_form = "SELECT MAX(id) as lastid FROM ag"; 
	...
	$sql = sprintf(
		"
		INSERT INTO `ag`
		( `id`, `name`, `phone`, `email`, `dob`, `risk` , `gender`, `rke`, `rkn`, `rkl` , `rks`, `rkf`, `time`, `active`, `ip`,`ti` )
		VALUES
		...
		mysqli_real_escape_string($conn,$_POST['rke']),
		mysqli_real_escape_string($conn,$_POST['rkn']), 
		mysqli_real_escape_string($conn,$_POST['rkl']),   
		mysqli_real_escape_string($conn,$_POST['rks']),  
		mysqli_real_escape_string($conn,$_POST['rkf']), 
		date("Y-m-d H:i:s"),
...
<?php
   ...
   $sql_form_rec = "SELECT * FROM ag Where  name = '$name' and dob = '$dob' and phone = '$phone' ";
