...
<?php
	...
	$q_id_form = "SELECT MAX(id) as lastid FROM tf"; 
	...
	$id_form = $row_id_form['lastid'];
     ...
     $sql = sprintf(
		"
		INSERT INTO `tf`
		( `id`, `name`, `phone`, `email`, `dob`, `risk` , `gender`, `dof`, `pk`, `pkf` , `fh`, `ks`, `time`, `active`, `ip`,`ti`)
		VALUES
		   ...
		   mysqli_real_escape_string($conn,$_POST['dof']),  
		   mysqli_real_escape_string($conn,$_POST['pk']),  
		   mysqli_real_escape_string($conn,$_POST['pkf']),   
		   mysqli_real_escape_string($conn,$_POST['fh']),
		   mysqli_real_escape_string($conn,$_POST['ks']), 
		   date("Y-m-d H:i:s"),"1"
...
$sql_form_rec = "SELECT * FROM tf Where  name = '$name' and dob = '$dob' and phone = '$phone' ";
