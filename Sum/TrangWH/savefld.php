...
<?php
	...
	$q_id_form = "SELECT MAX(id) as lastid FROM fld"; 
	...
	$id_form = $row_id_form['lastid'];
     ...
     $sql = sprintf(
		"
		INSERT INTO `fld`
		( `id`, `name`, `phone`, `email`, `dob`, `risk` , `gender`, `stm`, `overS`, `wl` , `lip`, `cho`, `hdl`, `dm`, `ps`, `time`, `active`, `ip`,`ti`)
		VALUES
		   ...
		   mysqli_real_escape_string($conn,$_POST['stm']),  
		   mysqli_real_escape_string($conn,$_POST['overS']),   
		   mysqli_real_escape_string($conn,$_POST['wl']),   
		   mysqli_real_escape_string($conn,$_POST['lip']),
		   mysqli_real_escape_string($conn,$_POST['cho']),  
		   mysqli_real_escape_string($conn,$_POST['hdl']),  
...
$sql_form_rec = "SELECT * FROM fld Where  name = '$name' and dob = '$dob' and phone = '$phone' ";
