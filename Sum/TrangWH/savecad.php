...
<?php
   ...
   $q_id_form = "SELECT MAX(id) as lastid FROM cad"; 
   ...
   $id_form = $row_id_form['lastid'];
    ...
    $sql = sprintf(
		"
		INSERT INTO `cad`
		( `id`, `name`, `phone`, `email`, `dob`, `risk` , `sex`, `sbp`, `cholesterol`, `dm` , `smk`, `time`,`active`,`ip`,`ti` )
		VALUES
		...
		mysqli_real_escape_string($conn,$_POST['sbp']),
		mysqli_real_escape_string($conn,$_POST['cholesterol']), 
		mysqli_real_escape_string($conn,$_POST['dm']), 
		mysqli_real_escape_string($conn,$_POST['smk']), 
		date("Y-m-d H:i:s"),"1",
...
$sql_form_rec = "SELECT * FROM cad Where  name = '$name' and dob = '$dob' and phone = '$phone' ";
