...
<?php
  ...
  if(!empty($_SERVER['HTTP_CLIENT_IP'])) {
   ...
   $q_id_form = "SELECT MAX(id) as lastid FROM member"; 
    ...
    $sql = sprintf(
	 "
	 INSERT INTO `member`
	 ( `id`, `name`, `phone`, `email`, `dob`, `weight` , `height`, `waisline`, `risk`, `gender` , `dm`, `pressure`, `time` ,`active`,`ip`,`ti`  )
	 VALUES
	   ...
	   mysqli_real_escape_string($conn,$_POST['weight']),   
	   mysqli_real_escape_string($conn,$_POST['height']), 
	   mysqli_real_escape_string($conn,$_POST['waistline']),
	   mysqli_real_escape_string($conn,$_POST['risk']),  
	   mysqli_real_escape_string($conn,$_POST['sex']),  
	   mysqli_real_escape_string($conn,$_POST['str']),  
	   mysqli_real_escape_string($conn,$_POST['pressure']),
	   date("Y-m-d H:i:s"),
...
$sql_form_rec = "SELECT * FROM member Where  name = '$name' and dob = '$dob' and phone = '$phone' ";
